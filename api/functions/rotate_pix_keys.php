 ...
 * 1. Identifica e exclui chaves ativas geradas há 30 minutos ou mais.
 * 2. Gera uma nova chave Pix aleatória (EVP) na API da Woovi.
 * 3. Define a nova chave como a chave padrão da conta.
 * 4. Registra a nova chave na tabela pix_key_rotations no SQLite.
 *
...
function rotate_pix_keys()
{
    ...
    try {
        ...
        // 1. Buscar chaves ativas criadas há 30 minutos ou mais (ou chaves antigas que ficaram pendentes de exclusão)
        $stmt_old = $db->prepare("
        ...
        // 2. Gerar nova chave aleatória (EVP) na Woovi
        ...
        // 3. Definir a nova chave como padrão na Woovi
        $default_res = woovi_set_default_pix_key($new_key);
        if (!$default_res['success']) {
            log_activity("Aviso: Não foi possível definir a chave {$new_key} como padrão imediatamente: " . ($default_res['error'] ?? ''), 'woovi_pix_key_rotation.log', __DIR__ . '/..');
        }

        // Desmarcar is_default de todas as chaves anteriores no SQLite
        $db->exec("UPDATE pix_key_rotations SET is_default = 0 WHERE is_default = 1");

        // 4. Salvar nova chave no banco de dados SQLite
        $ins = $db->prepare("
